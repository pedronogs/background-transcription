...
# ------------------- Stream from microphone to DeepSpeech using VAD ----------------------- #
    ...
    stream_context = model.createStream() # Start streaming to deepspeech' model
    wav_data = bytearray() # Initialize byte array to store audio data

    for frame in frames:
        if frame is not None: # If frame is not silent
            if spinner:
                spinner.start()
            if ARGS.remote:
        ...
        else: # If frame is silent (last frame)
            if spinner:
                spinner.stop()
            if ARGS.remote: # Upload file to API and transcribe remotelly
                vad_audio.write_wav('./upload.wav', wav_data) # Saves audio data to .wav file
                response = requests.post(ARGS.remote, files={'wavfile': open('upload.wav', 'rb')}) # Request to an API to transcribe audio
                text = response.json()['output']

                # Delete file after upload
                if os.path.exists("upload.wav"):
                    os.remove("upload.wav")

                wav_data = bytearray()
            else: # Transcribe locally
                text = stream_context.finishStream()
                print("Recognized: %s" % text)
                stream_context = model.createStream()

            execute_transcribed_command(text)
...
def execute_transcribed_command(command):
    ...
    confiability_array = []
    for key in commands_dictionary:
        confiability = fuzz.ratio(key, command)
        confiability_array.append(confiability)

    # Calculates biggest confiability index and checks if its above threshold
    max_confidence_index = confiability_array.index(max(confiability_array))
    if confiability_array[max_confidence_index] > 50:
        chosen_command = list(commands_dictionary)[max_confidence_index]
